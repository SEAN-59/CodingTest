...
/*
 제목: [3차] 파일명 정렬
 난도: Lv.2 / 완료한 사람: 약 11,000명 / 정답률: 57%
...
func solution(_ files:[String]) -> [String] {
    var result = [String]()
    var fileArr = [FileCase]()
    var count = 0
    
    files.forEach {
        var fileCase = FileCase(head: "", number: 0, count: 0)
        var numCount = 0
        var str = ""
        var numStr = ""
        ...
        $0.forEach {
            if let num = Int("\($0)") {
                if numCount == 0 { fileCase.head = str}
                if numStr.count < 5 { numStr += "\($0)" }
                else { fileCase.number = Int(numStr)! }
            }
            else {
                str += "\($0)".uppercased()
                if !numStr.isEmpty { fileCase.number = Int(numStr)! }
            }
        }
        fileCase.count = count
        count += 1
        fileArr.append(fileCase)
    }
    
    
    for i in 0 ..< fileArr.count {
        for j in 0 ..< fileArr.count - i - 1 {
            if fileArr[j].head > fileArr[j+1].head {
                let big = fileArr[j]
                fileArr[j] = fileArr[j+1]
                fileArr[j+1] = big
            } else if fileArr[j].head == fileArr[j+1].head {
                if fileArr[j].number > fileArr[j+1].number {
                    let big = fileArr[j]
                    fileArr[j] = fileArr[j+1]
                    fileArr[j+1] = big
                }
    ...
    for i in 0 ..< fileArr.count { result.append(files[fileArr[i].count]) }
    return result
}


print("A = \(solution( ["img12.png", "img10.png", "img02.png", "img1.png", "IMG01.GIF", "img2.JPG","img100009.png"]))")
print("A = \(solution( ["img12.png", "img10.png", "img02.png", "img1.png", "IMG01.GIF", "img2.JPG","img0000100009.png"]))")
print("A = \(solution( ["muzi1.txt", "MUZI1.txt", "muzi001.txt", "muzi1.TXT"]))")
